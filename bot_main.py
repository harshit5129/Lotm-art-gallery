...
def get_headers(bot_token: str) -> Dict[str, str]:
...
def get_channels(guild_id: str, bot_token: str) -> List[Dict[str, Any]]:
    try:
        return make_request(
...
def count_images_in_channel(channel_id: str, bot_token: str) -> int:
    image_count = 0
    last_message_id = None

    while True:
        messages = get_channel_messages(channel_id, bot_token, before=last_message_id)
        if not messages:
            break
    ...
    if count_images:
        for channel in channels:
            ...
            if channel_type in config.TEXT_CHANNEL_TYPES:
                image_counts[channel_id] = count_images_in_channel(channel_id, bot_token)
...
def select_channel(channels: List[Dict[str, Any]], bot_token: str, count_images: bool = False) -> Optional[Dict[str, Any]]:
...
def transfer_images_batch(
    ...
    with Progress(
        ...
        while True:
            messages = get_channel_messages(source_channel_id, bot_token, before=last_message_id)
            if not messages:
                break
...
def _handle_server_transfer(channel_id: str, bot_token: str, media_base_dir: str) -> None:
    ...
    source_channel = select_channel(source_channels, bot_token, count_images=False)
...
def download_menu(bot_token: str, media_base_dir: str) -> None:
...
def _download_from_channel(guilds: List[Dict[str, Any]], bot_token: str, media_base_dir: str) -> None:
    selected_guild = select_server(guilds)
    if not selected_guild:
        return
    ...
    if selected_channel:
        ...
        if confirm_action("Start download?"):
            stats = retrieve_messages(selected_channel["id"], bot_token, media_base_dir)
...
def transfer_menu(bot_token: str, media_base_dir: str) -> None:
    ...
    if choice == "0":
        ...
        _transfer_from_local(bot_token, media_base_dir)
    elif choice == "2":
...
def _transfer_from_server(bot_token: str, media_base_dir: str) -> None:
    ...
    target_channels = get_channels(target_guild["id"], bot_token)
    target_channel = select_channel(target_channels, bot_token, count_images=False)
...
def _transfer_whole_server(bot_token: str, media_base_dir: str) -> None:
    ...
    if transfer_option == "0":
        ...
        _transfer_to_single_channel(source_text_channels, target_guild, bot_token, media_base_dir)
    elif transfer_option == "2":
        _transfer_to_category(source_text_channels, target_guild, bot_token, media_base_dir)
    else:
        console.print("[red]Invalid option[/red]")
...
def interactive_menu(bot_token: str, media_base_dir: str) -> None:
    ...
    while True:
        ...
        if choice in menu_options:
            ...
            handler(bot_token, media_base_dir)
        else:
            console.print("[red]Invalid option[/red]")


def list_servers(bot_token: str) -> None:
    console.print(Panel.fit("[bold green]Discord Art Gallery Manager (Bot)[/bold green]", padding=1))
    guilds = get_guilds(bot_token)
    if not guilds:
        console.print("[red]No guilds found. Check your bot token.[/red]")
...
def list_channels(guild_id: str, bot_token: str) -> None:
    channels = get_channels(guild_id, bot_token)
    if not channels:
        console.print("[red]No channels found.[/red]")
        return
    display_channels(channels, bot_token, count_images=False)


def download_images(channel_id: str, bot_token: str, base_directory: str) -> None:
    console.print(f"\n[cyan]Downloading images from channel: {channel_id}[/cyan]")
    stats = retrieve_messages(channel_id, bot_token, base_directory)
...
def transfer_images(source_dir: str, target_channel_id: str, bot_token: str) -> None:
    media_folder = Path(source_dir)
    if not media_folder.exists():
        console.print(f"[red]Directory not found: {source_dir}[/red]")
        return
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Discord Art Gallery Manager (Bot)")
